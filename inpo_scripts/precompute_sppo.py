...
def add_probability_metrics(dataset: Dataset, num_proc: int = 12) -> Dataset:
    """
    根据 'all_rm_scores' 实时计算概率矩阵，然后计算并添加基于概率的指标。
    这个函数会为数据集新增四列: 'probability', 'chosen_probs', 'chosen_probs_win', 'chosen_probs_lose'.
    """

    def _compute_metrics(example: Dict) -> Dict:
        """处理单个数据行以计算所有概率指标。"""
        # 从 'all_rm_scores' 获取分数
        scores = np.array(example['all_rm_scores'])
        num_responses = len(scores)

        # 步骤1: 实时计算概率矩阵
        prob_matrix = np.zeros((num_responses, num_responses))
        for i in range(num_responses):
            for j in range(num_responses):
                prob_matrix[i, j] = 1 / (1 + np.exp(scores[j] - scores[i]))

        # 步骤2: 找到最高分和最低分的索引
        chosen_idx = scores.argmax()
        rejected_idx = scores.argmin()

        # 步骤3: 计算综合获胜概率
        # .sum(axis=1) 计算每个回答优于其他所有回答的概率总和
        metrics_prob = prob_matrix.sum(axis=1)

        # 步骤4: 计算三个核心指标
        chosen_probs = prob_matrix[chosen_idx, rejected_idx]
        chosen_probs_win = metrics_prob[chosen_idx] / num_responses
        chosen_probs_lose = metrics_prob[rejected_idx] / num_responses

        # 返回一个包含所有新列的字典
        return {
            "probability": prob_matrix.tolist(),  # 将矩阵也加入，方便未来使用
            "chosen_probs": chosen_probs,
            "chosen_probs_win": chosen_probs_win,
            "chosen_probs_lose": chosen_probs_lose,
        }

    # 使用 .map() 高效地将计算应用到整个数据集
    return dataset.map(_compute_metrics, num_proc=num_proc)
...
def main():
    ...
    logger.info("Calculating probability matrix from 'all_rm_scores' and adding metrics...")
    raw_dataset = add_probability_metrics(raw_dataset, num_proc=12)
    logger.info("Probability metrics calculated and added successfully.")
