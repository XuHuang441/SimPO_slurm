...
def extract_user_prompt(formatted_prompt: str) -> str:
    """
    Extracts the clean user prompt from the formatted string.
    Example input: "<bos><start_of_turn>user\nPROMPT<end_of_turn>..."
    """
    # Use regular expression to find the content between 'user\n' and '<end_of_turn>'
    match = re.search(r"user\n(.*?)\n*<end_of_turn>", formatted_prompt, re.DOTALL)
    if match:
        # Strip leading/trailing whitespace from the extracted prompt
